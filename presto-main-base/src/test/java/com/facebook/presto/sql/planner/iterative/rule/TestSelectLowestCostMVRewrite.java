 ...
 *
 * FilterNodes are given explicit PlanNodeIds (origFilter, mv1Filter, etc.)
 * so we can verify which candidate was selected after GroupReference resolution.
 */
...
public class TestSelectLowestCostMVRewrite
{
    ...
    public void testSelectsCheapestAmongMultipleCandidates()
    {
                    ...
                    return new MVRewriteCandidatesNode(
                            ...
                            ImmutableList.of(
                                    new MVRewriteCandidate(
                                            ...
                                            "catalog", "schema", "mv1"),
                                    new MVRewriteCandidate(
                                            p.filter(new PlanNodeId("mv2Filter"), TRUE_CONSTANT, p.values(new PlanNodeId("mv2Src"), 1, col)),
                                            "catalog", "schema", "mv2"),
                                    new MVRewriteCandidate(
                                            p.filter(new PlanNodeId("mv3Filter"), TRUE_CONSTANT, p.values(new PlanNodeId("mv3Src"), 1, col)),
                                            "catalog", "schema", "mv3")),
                            ImmutableList.of(col));
                })
                .get();
        ...
        assertTrue(result instanceof FilterNode);
        assertEquals(result.getId(), new PlanNodeId("mv2Filter"));
    }
        ...
        PlanNode result = tester.assertThat(new SelectLowestCostMVRewrite(COST_COMPARATOR))
                ...
                .on(p -> {
                    ...
                    return new MVRewriteCandidatesNode(
                            ...
                            p.filter(new PlanNodeId("origFilter"), TRUE_CONSTANT, p.values(new PlanNodeId("origSrc"), 1, col)),
                            ImmutableList.of(new MVRewriteCandidate(
                                    p.filter(new PlanNodeId("mv1Filter"), TRUE_CONSTANT, p.values(new PlanNodeId("mv1Src"), 1, mvCol)),
                                    "catalog", "schema", "mv1")),
                            ImmutableList.of(col));
                })
                .get();
    ...
    {
        try (RuleTester disabledTester = new RuleTester(ImmutableList.of(), ImmutableMap.of(
                "materialized_view_query_rewrite_cost_based_selection_enabled", "false"), Optional.of(NODES_COUNT))) {
